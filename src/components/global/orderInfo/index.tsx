...
const OrderInfo = ({ order }: OrderDetailsModalProps) => {
  ...
  const statusOptions = ["Placed", "Shipped", "Delivered", "Returned", "Cancelled"]
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex gap-4">
                  <Avatar className="w-24 h-24 object-cover relative rounded-lg shrink-0">
                    ...
                    />
                    <AvatarFallback>{order.product.productName.slice(0, 2)}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1 space-y-1">
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                      ...
                      <div className="flex items-center gap-1">
                        <Package size={14} />
                        <span>Qty: {order.product.qty || order.qty || 1}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Calendar size={14} />
                        <span>Placed: {formatDate(order.orderPlacedOn)}</span>
                      </div>
                        ...
                        <SelectContent>
                          {statusOptions.map((option) => (
                            <SelectItem key={option} value={option}>
                              <Badge className={`${getStatusColor(option)} text-white`}>{option}</Badge>
                            </SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      ...
                      <div className="mt-4">
                        ...
                        {order.paymentDetails.map((payment, index) => (
                          <div key={index} className="bg-muted p-3 rounded-md text-sm">
                            ...
                            <div className="flex justify-between">
                              <span>Status</span>
                              <Badge variant={payment.state === "success" ? "default" : "default"}>
                                {payment.state}
                              </Badge>
                            </div>
                          </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{formatDate(order.orderPlacedOn)}</p>
                        </div>
                      </div>
                      ...
                      {order.shippedOn && (
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{formatDate(order.shippedOn)}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {order.deliveredOn && (
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{formatDate(order.deliveredOn)}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {order.returnedOn && (
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <p className="font-medium text-sm">Order Returned</p>
                            <p className="text-xs text-muted-foreground">{formatDate(order.returnedOn)}</p>
                          </div>
                        </div>
                      )}
                      ...
                      {order.cancelledOn && (
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">{formatDate(order.cancelledOn)}</p>
                          </div>
                        </div>
                      )}

                      {!order.shippedOn && !order.deliveredOn && !order.returnedOn && !order.cancelledOn && (
                        <p className="text-muted-foreground italic text-sm">No additional status updates available.</p>
                      )}
                    </div>
                  </CardContent>
                </Card>
