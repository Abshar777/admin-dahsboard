    ...
    <>
     <ScrollArea className="max-h-[600px] space-y-4">
          <div className="flex gap-4">
             ...
             <Avatar className="w-32 h-32 object-cover rounded-lg">
              <AvatarImage className="object-cover"  src={`${process.env.NEXT_PUBLIC_BACKEND_URL}/${order.product.images.image1}`} alt={order.product.productName} />
              <AvatarFallback>alt={order.product.productName.split("").splice(0,2).join("")}</AvatarFallback>
            </Avatar>
            <div className="flex-1">
              ...
              <p><strong>Quantity:</strong> {order.product.qty}</p>
              <p className="font-semibold flex items-center gap-2">
                <Calendar size={16} /> Order Placed On: {new Date(order.orderPlacedOn).toLocaleDateString()}
              </p>
          ...
          <div className="flex mb-2 mt-2 items-center gap-4 p-4 bg-muted/20 rounded-lg">
            <Avatar>
              <AvatarImage src={order.user?.img || "https://via.placeholder.com/50"} alt={order.user.username} />
          ...
          <div className="p-4 mb-2 bg-muted/20 rounded-lg">
            <h3 className="font-semibold flex items-center gap-2"><MapPin size={16} /> Shipping Address</h3>
