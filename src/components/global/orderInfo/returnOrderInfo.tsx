...
import { useEditOrder, useUpdateReturnedOrder } from "@/hooks/useOrder";
...
const ReturnOrderInfo = ({ order }: OrderDetailsModalProps) => {
  const [status, setStatus] = useState(order.status);
  const { mutate, isPending } = useUpdateReturnedOrder({ id: order._id });
  const { mutate: createChatFn, isPending: chatPending } = useCreateChat();

  const statusOptions = ["pending", "approved", "rejected", "notApplicable"];
      ...
      <div className="p-4 space-y-6">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-xl">Return Order Information</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="flex-1 space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Package size={16} />
                  <span>
                    Quantity: {order?.product?.qty || order?.qty || 1}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <Calendar size={16} />
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex gap-4">
                <Avatar>
                  <AvatarImage
                    src={
                      order?.user?.img || "/placeholder.svg?height=50&width=50"
                    }
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Badge
                      variant={
                        order?.isPaid === "true" ? "default" : "destructive"
                      }
                    >
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {formatDate(order.orderPlacedOn)}
                      </p>
                    </div>
                  </div>
                  ...
                  {!order?.shippedOn &&
                    ...
                    !order?.cancelledOn && (
                      <p className="text-muted-foreground italic">
                        No additional status updates available.
                      </p>
                    )}
                </div>
              </CardContent>
