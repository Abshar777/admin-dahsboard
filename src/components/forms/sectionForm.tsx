...
import { useSectionCreation } from "@/hooks/useSections";
...
const SectionForm = (props: Props) => {
 const {emblaRef,scrollNext,scrollPrev}=useCarsole()
  ...
  const { products, form, onFormSubmit ,isPending,isLoading} = useSectionCreation();
                  ...
                  <div className="overflow-hidden " ref={emblaRef}>
                    <div className="flex w-full">
                      {products.map((product, index) => (
                        <div key={index} className="p-1 relative min-w-[20%]">
                          ...
                          <ProductCard
                            onClick={() =>
                              !field.value.includes(product._id)
                                ...
                                : field.onChange(
                                    field.value.filter((e) => e !== product._id)
                                  )
                            }
                            className={cn(
                              " active:scale-[.9]",
                              field.value.includes(product._id) && "opacity-50"
                            )}
